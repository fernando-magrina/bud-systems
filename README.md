...
When you clone the repo, some versions of Visual Studio show a fake fullbench-dll.sln entry in Solution Explorer. It does not exist on disk and cannot be removed. Please ignore it if this is the case.  
...
⚠️ Just in case it does not load automatically, you can right-click on the solution and set it manually.
...
The .spec files included in the Visual Studio Angular template were not working, so I replaced them with simple .test.ts files that run using npm test.  
This kept the client-side tests working without needing to fix the template. 
Please use the Developer PowerShell in Visual Studio to run those tests
